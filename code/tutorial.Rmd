...
```{r, eval=FALSE, results=FALSE, warning=FALSE, message=FALSE}
iris_tbl %>% 
  ft_string_indexer("Species", "species_inx")
```

Some machine learning algorithms require numerical columns instead of string columns. That's why function `ft_string_indexer()` is very useful. It encodes a column of labels into a column of label indices. The indices are in [0, numLabels), ordered by label frequencies, with the most frequent label assigned index 0. The transformation can be reversed with `ft_index_to_string()`.
...
```{r, eval=FALSE, results=FALSE, warning=FALSE, message=FALSE}
# first we add column 'species_inx'
iris_tbl_inx <- iris_tbl %>% 
  ft_string_indexer("Species", "species_inx")

# split data on training and testing set
train_test <- iris_tbl_inx %>% 
...
iris_tbl_inx2 <- iris_tbl %>% 
  filter(Species != "virginica") %>% 
  ft_string_indexer("Species", "species_inx")

train_test2 <- iris_tbl_inx2 %>% 
  sdf_partition(training = 0.7, testing = 0.3)
...
model_logr <- training_tbl2 %>%
  ml_logistic_regression(Species ~ Petal_Length + Petal_Width)
...
model_dt <- training_tbl %>%
  ml_decision_tree(Species ~ Petal_Length + Petal_Width)

# predict classes for testing dataset
predict_dt <- sdf_predict(model_dt, testing_tbl) %>%
  collect

# compare predicted classes with real classess
table(predict_dt$Species_idx, predict_dt$prediction)
...
# TODO:
zamiast iris_tbl powinno chyba byc trainig_tbl
Doda ft_string_indexer!
logistic regression zrobic na iris ale ograniczyc sie tylko do dwoch klas
random forest i dt - prediction jak zroboc zeby tabelka byla ok
ft_indexer zrobić przed podziałem na test i tren
titanic do workflow
